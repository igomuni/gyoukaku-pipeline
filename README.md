# 行政事業レビューシート データ処理パイプライン API v1.3.1

## 概要

本プロジェクトは、日本の各府省が公開する「行政事業レビューシート」のExcel/ZIPファイルから構造化データを抽出し、クリーンなCSVファイルを生成するデータパイプラインと、その生成物を活用して自然言語からSQLを生成するText-to-SQLの実験・評価を行うためのWeb UIツールを提供します。
...
- **豊富な履歴管理**:
    ...
    - **再実行機能**: 履歴をクリック後、内容を変更せずに実行すると、新しい履歴を追加せず既存の履歴を更新（実行回数インクリメント、タイムスタンプ更新）します。
